...
import DevolucionParcialDialog from '../../components/modules/ventas/DevolucionParcialDialog';
...
const DetalleVenta = () => {
  ...
  const [showTicketPago, setShowTicketPago] = useState(false);
  ...
  const registrarPago = async (pagoData) => {
    try {
      ...
      console.log('🔄 Obteniendo datos actualizados de la venta después del pago en DetalleVenta...');
      const ventaActualizada = await ventasService.obtenerPorId(venta.id);
      console.log('✅ Venta actualizada obtenida en DetalleVenta:', ventaActualizada);
      
      // Actualizar el estado de la venta con los datos actualizados
  ...
  /**
   * Procesa una devolución parcial
   * @param {Array} productosDevolver - Productos a devolver
   * @param {string} motivo - Motivo de la devolución
   */
  const procesarDevolucionParcial = async (productosDevolver, motivo) => {
    try {
      await ventasService.procesarDevolucionParcial(venta.id, productosDevolver, motivo);
      ...
      toast.error(error.message || 'Error al procesar la devolución parcial');
      throw error;
    }
  };
  ...
  const getColorEstado = (estado) => {
	...
	const columnsProductos = [
	  ...
	  },
	  {
		...
		accessor: 'cantidad',
		cell: (row) => {
		  const cantidadTotal = row.cantidadTotal || row.cantidad;
		  const unidadesGratis = row.unidadesGratis || 0;
		  const cantidadOriginal = row.cantidadOriginal || row.cantidad;
		    ...
		    <span className="font-medium">
		      ...
		      {unidadesGratis > 0 && (
		        <span className="text-green-600 text-sm ml-1">
		          ({cantidadOriginal}+{unidadesGratis} gratis)
		        </span>
		      )}
		    </span>
		  );
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-6">
          ...
          <Card title="Resumen" icon={<FaMoneyBillWave />}>
            <div className="space-y-3">
              ...
              <div className="pt-3 border-t">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold text-lg text-indigo-600">
                    {formatMoneda(venta.total || venta.subtotal)}
                  </span>
                </div>
              </div>
              
              {/* Mostrar información de pagos si hay pagos parciales */}
              ...
              {/* Botón de registrar pago - siempre que haya saldo pendiente */}
              {(venta.saldo_pendiente > 0 || (venta.total - venta.total_pagado) > 0) && (
                <div className="pt-3">
                  <Button
                    color="primary"
                    fullWidth
              ...
              {/* Botón para imprimir comprobante de pago - solo para ventas pagadas */}
              {venta.saldo_pendiente === 0 && venta.total_pagado > 0 && (
                <div className="pt-3">
                  <Button
                    color="success"
                    fullWidth
      ...
      <TicketReciboPago
        ...
        onClose={() => setShowComprobantePago(false)}
        pagoData={null}
        venta={venta}
        cliente={venta?.cliente_info}
        modo="venta_pagada"
