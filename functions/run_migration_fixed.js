// functions/run_migration_fixed.js - SCRIPT EJECUTABLE CON PROJECT ID

const admin = require('firebase-admin');
const fs = require('fs');
const path = require('path');

// ==================== DETECTAR PROJECT ID AUTOMÁTICAMENTE ====================
function detectarProjectId() {
  try {
    // Método 1: Leer desde .firebaserc
    const firebaseRcPath = path.join(__dirname, '..', '.firebaserc');
    if (fs.existsSync(firebaseRcPath)) {
      const firebaseRc = JSON.parse(fs.readFileSync(firebaseRcPath, 'utf8'));
      if (firebaseRc.projects && firebaseRc.projects.default) {
        console.log(`📋 Project ID detectado desde .firebaserc: ${firebaseRc.projects.default}`);
        return firebaseRc.projects.default;
      }
    }

    // Método 2: Leer desde firebase.json
    const firebaseJsonPath = path.join(__dirname, '..', 'firebase.json');
    if (fs.existsSync(firebaseJsonPath)) {
      const firebaseJson = JSON.parse(fs.readFileSync(firebaseJsonPath, 'utf8'));
      if (firebaseJson.projectId) {
        console.log(`📋 Project ID detectado desde firebase.json: ${firebaseJson.projectId}`);
        return firebaseJson.projectId;
      }
    }

    // Método 3: Variable de entorno
    if (process.env.FIREBASE_PROJECT_ID) {
      console.log(`📋 Project ID detectado desde variable de entorno: ${process.env.FIREBASE_PROJECT_ID}`);
      return process.env.FIREBASE_PROJECT_ID;
    }

    // Si no se encuentra, pedir al usuario
    console.log('⚠️ No se pudo detectar automáticamente el Project ID');
    console.log('🔧 Opciones:');
    console.log('1. Ejecutar: firebase use --add');
    console.log('2. O editar este script y agregar tu Project ID manualmente');
    
    // AGREGAR TU PROJECT ID AQUÍ SI ES NECESARIO
    // return 'tu-project-id-aqui';
    
    throw new Error('la-fabrica-1');

  } catch (error) {
    console.error('❌ Error al detectar Project ID:', error.message);
    throw error;
  }
}
...
let projectId;
try {
  projectId = detectarProjectId();
  
  admin.initializeApp({
    projectId: projectId,
    // Las credenciales se obtienen automáticamente del SDK
  });
  
  console.log(`✅ Firebase Admin inicializado correctamente para proyecto: ${projectId}`);
} catch (error) {
  console.error('❌ Error al inicializar Firebase Admin:', error.message);
  console.log('\n🔧 SOLUCIONES:');
  console.log('1. Ejecutar: firebase login');
  console.log('2. Ejecutar: firebase use --add');
  console.log('3. O editar este script línea 45 con tu Project ID');
  process.exit(1);
}

const db = admin.firestore();

// ==================== FUNCIONES DE MIGRACIÓN (IGUALES QUE ANTES) ====================
  ...
  try {
    switch (comando) {
      ...
      default:
        ...
        console.log('   node run_migration_fixed.js preview     - Ver qué se migrará (sin ejecutar)');
        console.log('   node run_migration_fixed.js verificar   - Verificar estado actual');
        console.log('   node run_migration_fixed.js migrar      - Ejecutar migración completa');
        console.log('');
        console.log('🔧 Ejemplos:');
        console.log('   node run_migration_fixed.js preview');
        console.log('   node run_migration_fixed.js migrar');
        break;
    }
